...
#write_csv(profiles, "/students/aabakhitova/spbRecommend/vk_posts.csv")

vk_posts = vk_posts %>% select(attachments.link, geo) %>%  select(id:reposts.count,  starts_with("attachments.link"),starts_with("geo")) 

vk_posts= vk_posts%>% select(-(attachments.link.title1:attachments.link.preview_url))
#write_csv(vk_posts, "/students/aabakhitova/spbRecommend/vk_posts_light.csv")
...
library(tidyr)
likes_vk = data.frame()
for (i in vk_posts$id[5347:10000]) {
  ...
  Sys.sleep(0.5)
  likes_vk = plyr::rbind.fill(likes_vk, temp)
}

likes_vk2 = likes_vk %>% unnest()
write.csv(likes_vk2, "~/spbRecommend/likes_vk10000.csv")
